...
export default function Register() {
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    if (!agreed) { showToast(t("pleaseAgreeTermsError"), "error"); return; }
    if (form.password.length < 8) { showToast(t("passwordTooShortError"), "error"); return; }
    setLoading(true);
    setTimeout(() => {
      setLoading(false);
  ...
  return (
    <div className="min-h-screen bg-background flex">
      <div className="hidden lg:flex lg:w-1/2 bg-hero flex-col justify-between p-12 relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-primary/20 to-hero" />
        ...
        <div className="relative space-y-4">
          <h2 className="text-3xl font-bold text-hero-foreground">{t("startJourneyTodayLabel")}</h2>
          <ul className="space-y-3">
            {[
              t("signupBulletMaterials"),
              t("signupBulletBookSessions"),
              t("signupBulletTrackProgress"),
              t("signupBulletOnlineTheory"),
            ].map((item, i) => (
              <li key={i} className="flex items-center gap-3 text-hero-foreground/80">
        ...
        <div className="w-full max-w-md">
          ...
          <div className="flex justify-between items-center mb-8">
            <div>
              ...
              <p className="text-muted-foreground text-sm mt-1">{t("joinViva")}</p>
            </div>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <Button type="submit" disabled={loading} className="w-full bg-primary hover:bg-primary/90 text-primary-foreground h-11 mt-2 disabled:opacity-70">
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
            ...
            >
              <GoogleIcon />
              {socialLoading === "google" ? t("redirecting") : t("continueWithGoogle")}
            </Button>
            <Button
              type="button"
              variant="outline"
            ...
            >
              <FacebookIcon />
              {socialLoading === "facebook" ? t("redirecting") : t("continueWithFacebook")}
            </Button>
            <Button
              type="button"
              variant="outline"
            ...
            >
              <AppleIcon />
              {socialLoading === "apple" ? t("redirecting") : t("continueWithAppleCloud")}
            </Button>
          </div>
