...
import { CAR_ASSIGNABLE_INSTRUCTORS } from "src/modules/instructors/car-assignable-instructors";
...
function instructorNamesForEmails(emails: string[] | undefined) {
  if (!emails?.length) return "—";
  return emails
    .map((e) => CAR_ASSIGNABLE_INSTRUCTORS.find((x) => x.email === e)?.name ?? e)
    .join(", ");
}
            ...
            <form onSubmit={handleSaveCar} className="space-y-3 mt-2">
              ...
              <div>
                ...
                <div className="rounded-md border border-border p-3 space-y-2 max-h-44 overflow-y-auto">
                  {CAR_ASSIGNABLE_INSTRUCTORS.map((ins) => {
