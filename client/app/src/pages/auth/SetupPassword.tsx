...
export default function SetupPassword() {
    ...
    void (async () => {
      try {
        const res = await vivaApiJson<{ valid: boolean; email: string }>(
          `/auth/student-invitation?token=${encodeURIComponent(token)}`,
        );
  ...
  const onSubmit = useCallback(
    async (e: React.FormEvent) => {
      e.preventDefault();
      if (!token) {
        showToast(t("setupPasswordTokenMissing"), "error");
        return;
      }
      ...
      setSubmitting(true);
      try {
        await vivaApiJson("/auth/setup-password", { method: "POST", body: { token, password } });
        showToast(t("setupPasswordSuccess"), "success");
        setLocation(absWouterHref("/login"));
      } catch (err) {
        showToast(getApiErrorMessage(err) || t("setupPasswordTokenMissing"), "error");
      } finally {
        setSubmitting(false);
      }
    },
    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <div className="bg-card rounded-2xl border border-border shadow-sm p-8">
          ...
          <h1 className="text-2xl font-bold text-foreground mb-2">{t("setupPasswordTitle")}</h1>
          <p className="text-muted-foreground text-sm mb-2">{t("setupPasswordSubtitle")}</p>
          {metaOk && maskedEmail ? (
          ...
          {!loadingMeta && !metaOk ? (
            <p className="text-sm text-destructive mb-6">{t("setupPasswordTokenMissing")}</p>
          ) : null}
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            >
              {submitting ? t("loading") : t("setupPasswordSubmit")}
            </Button>
          </form>
