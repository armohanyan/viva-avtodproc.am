...
export default function InstructorPanelLayout({ children }: Props) {
	...
	const nav = useMemo(() => {
		const links = filterInstructorNavLinks(INSTRUCTOR_NAV_LINKS, { teachesPractical, teachesTheory });
		return links.map((link) => ({
			href: link.href,
			icon: iconByPath[link.href] ?? LayoutDashboard,
			label: t(link.translationKey),
		}));
	}, [t, teachesPractical, teachesTheory]);
	...
	const headerTitle = pathHasPrefix(location, "/instructor/notifications")
		? t("notifications")
		: pathHasPrefix(location, "/instructor/questions")
			? t("instructorQuestionsNav")
