...
import {
  parsePetrolExpenseWorkbook,
  toPetrolExpenseBulkPayload,
  type ParsedPetrolExpenseRow,
} from "src/modules/admin/petrol/excelPetrolExpenseImport";

export type ExcelPetrolExpenseImportModalProps = {
  open: boolean;
...
export default function ExcelPetrolExpenseImportModal({
  open,
  onOpenChange,
  cars,
  instructors,
  onImported,
}: ExcelPetrolExpenseImportModalProps) {
  const { t, lang } = useLang();
  const { showToast } = useToast();
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [parsing, setParsing] = useState(false);
  const [importing, setImporting] = useState(false);
  const [rows, setRows] = useState<ParsedPetrolExpenseRow[]>([]);
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const result = await parsePetrolExpenseWorkbook(file, cars, instructors);
      setRows(result.rows);
  ...
  const handleConfirmImport = async () => {
    ...
    const payloads = selectedValidRows
      .map(toPetrolExpenseBulkPayload)
  ...
  return (
    <>
      ...
      <AppModal
        ...
        title={t("adminPetrolExpenseImportModalTitle")}
                ...
                <table className="w-full min-w-[720px] text-sm">
                  <thead className="sticky top-0 bg-muted/80 backdrop-blur">
                    <tr className="border-b text-left">
                      ...
                      <th className="px-3 py-2 text-right">{t("adminPetrolColCount")}</th>
