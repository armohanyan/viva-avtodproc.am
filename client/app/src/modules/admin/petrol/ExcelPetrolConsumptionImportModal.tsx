...
import { bulkImportPetrolConsumptions } from "src/modules/admin/petrol/adminPetrol.api";
import {
  parsePetrolConsumptionWorkbook,
  toPetrolConsumptionBulkPayload,
  type ParsedPetrolConsumptionRow,
} from "src/modules/admin/petrol/excelPetrolConsumptionImport";

export type ExcelPetrolConsumptionImportModalProps = {
  open: boolean;
...
export default function ExcelPetrolConsumptionImportModal({
  open,
  onOpenChange,
  cars,
  instructors,
  onImported,
}: ExcelPetrolConsumptionImportModalProps) {
  const { t, lang } = useLang();
  const { showToast } = useToast();
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [parsing, setParsing] = useState(false);
  const [importing, setImporting] = useState(false);
  const [rows, setRows] = useState<ParsedPetrolConsumptionRow[]>([]);
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const result = await parsePetrolConsumptionWorkbook(file, cars, instructors);
      setRows(result.rows);
  ...
  const handleConfirmImport = async () => {
    ...
    const payloads = selectedValidRows
      .map(toPetrolConsumptionBulkPayload)
      .filter((p): p is NonNullable<typeof p> => p != null);
    if (payloads.length === 0) return;

    setImporting(true);
    try {
      const summary = await bulkImportPetrolConsumptions(payloads);
  ...
  return (
    <>
      ...
      <AppModal
        ...
        title={t("adminPetrolConsumptionImportModalTitle")}
      ...
      >
        <div className="space-y-4">
          <div className="flex flex-wrap items-center gap-3">
            ...
            {fileLabel ? <span className="text-sm text-muted-foreground truncate max-w-[20rem]">{fileLabel}</span> : null}
          </div>
                ...
                <table className="w-full min-w-[760px] text-sm">
                  <thead className="sticky top-0 bg-muted/80 backdrop-blur">
                    <tr className="border-b text-left">
                      ...
                      <th className="px-3 py-2 text-right">{t("adminPetrolConsumptionColDistance")}</th>
                      <th className="px-3 py-2 text-right">{t("adminPetrolConsumptionColPetrol")}</th>
                    ...
                    {rows.map((row) => (
                      ...
                      >
                        ...
                        <td className="px-3 py-2 text-right tabular-nums">
                          {row.distanceValue} {row.distanceUnit}
                        </td>
                        <td className="px-3 py-2 text-right tabular-nums">
                          {row.petrolAmount != null ? `${row.petrolAmount} ${row.petrolUnit}` : "—"}
                        </td>
                        <td className="px-3 py-2 text-xs">
                          {row.isExample ? (
                            <span className="text-muted-foreground">{t("adminPetrolImportExampleRow")}</span>
                          ) : row.valid ? (
                            <span className="text-emerald-600 dark:text-emerald-400">{t("adminPetrolImportStatusOk")}</span>
                          ) : (
